...
namespace Lab1.accounts
{
    public class BoostedAccount : GameAccount
    {
        ...
        private bool HasWinStrickOf(int number)
        {
            int totalGames = games.Count;
            if (totalGames < number)
                return false;

            int counter = 0;
            while (counter < number)
            {
                // iterate over "number" last games of the player.
